package com.example.cylindercloud.websocket.protocol.module;

import java.io.Serializable;

/**
 * Created by wxj on 2015-9-18.
 */
public class CylinderInfo implements Serializable {
    private String azbgbh;
    private String azdw;
    private String azrq;
    private String bdrq;
    private String bfnx;
    private String bfrq;
    private String ccbh;
    private String ccrq;
    private String cphm;
    private String cqdm;
    private String cqdw;
    private String cqjz;
    private String factureReName;
    private String fzjg;
    private String fzrq;
    private String fzry;
    private String gcyl;
    private String gd;
    private String gzwd;
    private String jcbgbh;
    private String jcdw;
    private String jcrq;
    private String jjpz;
    private String jjpzrq;
    private String jjrq;
    private String jjsh;
    private String jjshrq;
    private String jjy;
    private String jyzq;
    private String mqzt;
    private String pressNo;
    private String qpzl;
    private String regUnit;
    private String rfid;
    private String rj;
    private String sjhd;
    private String sjjyzq;

    public String getAzbgbh() {
        return azbgbh;
    }

    public void setAzbgbh(String azbgbh) {
        this.azbgbh = azbgbh;
    }

    public String getAzdw() {
        return azdw;
    }

    public void setAzdw(String azdw) {
        this.azdw = azdw;
    }

    public String getAzrq() {
        return azrq;
    }

    public void setAzrq(String azrq) {
        this.azrq = azrq;
    }

    public String getBdrq() {
        return bdrq;
    }

    public void setBdrq(String bdrq) {
        this.bdrq = bdrq;
    }

    public String getBfnx() {
        return bfnx;
    }

    public void setBfnx(String bfnx) {
        this.bfnx = bfnx;
    }

    public String getBfrq() {
        return bfrq;
    }

    public void setBfrq(String bfrq) {
        this.bfrq = bfrq;
    }

    public String getCcbh() {
        return ccbh;
    }

    public void setCcbh(String ccbh) {
        this.ccbh = ccbh;
    }

    public String getCcrq() {
        return ccrq;
    }

    public void setCcrq(String ccrq) {
        this.ccrq = ccrq;
    }

    public String getCphm() {
        return cphm;
    }

    public void setCphm(String cphm) {
        this.cphm = cphm;
    }

    public String getCqdm() {
        return cqdm;
    }

    public void setCqdm(String cqdm) {
        this.cqdm = cqdm;
    }

    public String getCqdw() {
        return cqdw;
    }

    public void setCqdw(String cqdw) {
        this.cqdw = cqdw;
    }

    public String getCqjz() {
        return cqjz;
    }

    public void setCqjz(String cqjz) {
        this.cqjz = cqjz;
    }

    public String getFactureReName() {
        return factureReName;
    }

    public void setFactureReName(String factureReName) {
        this.factureReName = factureReName;
    }

    public String getFzjg() {
        return fzjg;
    }

    public void setFzjg(String fzjg) {
        this.fzjg = fzjg;
    }

    public String getFzrq() {
        return fzrq;
    }

    public void setFzrq(String fzrq) {
        this.fzrq = fzrq;
    }

    public String getFzry() {
        return fzry;
    }

    public void setFzry(String fzry) {
        this.fzry = fzry;
    }

    public String getGcyl() {
        return gcyl;
    }

    public void setGcyl(String gcyl) {
        this.gcyl = gcyl;
    }

    public String getGd() {
        return gd;
    }

    public void setGd(String gd) {
        this.gd = gd;
    }

    public String getGzwd() {
        return gzwd;
    }

    public void setGzwd(String gzwd) {
        this.gzwd = gzwd;
    }

    public String getJcbgbh() {
        return jcbgbh;
    }

    public void setJcbgbh(String jcbgbh) {
        this.jcbgbh = jcbgbh;
    }

    public String getJcdw() {
        return jcdw;
    }

    public void setJcdw(String jcdw) {
        this.jcdw = jcdw;
    }

    public String getJcrq() {
        return jcrq;
    }

    public void setJcrq(String jcrq) {
        this.jcrq = jcrq;
    }

    public String getJjpz() {
        return jjpz;
    }

    public void setJjpz(String jjpz) {
        this.jjpz = jjpz;
    }

    public String getJjpzrq() {
        return jjpzrq;
    }

    public void setJjpzrq(String jjpzrq) {
        this.jjpzrq = jjpzrq;
    }

    public String getJjrq() {
        return jjrq;
    }

    public void setJjrq(String jjrq) {
        this.jjrq = jjrq;
    }

    public String getJjsh() {
        return jjsh;
    }

    public void setJjsh(String jjsh) {
        this.jjsh = jjsh;
    }

    public String getJjshrq() {
        return jjshrq;
    }

    public void setJjshrq(String jjshrq) {
        this.jjshrq = jjshrq;
    }

    public String getJjy() {
        return jjy;
    }

    public void setJjy(String jjy) {
        this.jjy = jjy;
    }

    public String getJyzq() {
        return jyzq;
    }

    public void setJyzq(String jyzq) {
        this.jyzq = jyzq;
    }

    public String getMqzt() {
        return mqzt;
    }

    public void setMqzt(String mqzt) {
        this.mqzt = mqzt;
    }

    public String getPressNo() {
        return pressNo;
    }

    public void setPressNo(String pressNo) {
        this.pressNo = pressNo;
    }

    public String getQpzl() {
        return qpzl;
    }

    public void setQpzl(String qpzl) {
        this.qpzl = qpzl;
    }

    public String getRegUnit() {
        return regUnit;
    }

    public void setRegUnit(String regUnit) {
        this.regUnit = regUnit;
    }

    public String getRfid() {
        return rfid;
    }

    public void setRfid(String rfid) {
        this.rfid = rfid;
    }

    public String getRj() {
        return rj;
    }

    public void setRj(String rj) {
        this.rj = rj;
    }

    public String getSjhd() {
        return sjhd;
    }

    public void setSjhd(String sjhd) {
        this.sjhd = sjhd;
    }

    public String getSjjyzq() {
        return sjjyzq;
    }

    public void setSjjyzq(String sjjyzq) {
        this.sjjyzq = sjjyzq;
    }
}
